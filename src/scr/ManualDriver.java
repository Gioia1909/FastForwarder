...
public Action control(SensorModel sensors) {
    ...
    System.out.print("[W/A/S/D] + invio per guidare: ");
    ...
    if (input.contains("w")) action.accelerate = 1;
    if (input.contains("s")) action.brake = 1;
    if (input.contains("a")) action.steering = -1;
    if (input.contains("d")) action.steering = 1;

    // Salva i dati in dataset.csv
